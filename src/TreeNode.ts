...
interface IControlNodeMetaType {
  [k: string]: any;
}
...
const CommonDesignerPropsMap = new Map<string, any>();
...
export class TreeNode {
  ...
  //   // const designerProps = GlobalRegistry.getComponentDesignerProps(this.componentName);
  //   const finallyDesignerProps: IDesignerProps = {};
  //   if (isFn(designerProps)) {
  //     Object.assign(finallyDesignerProps, designerProps(this), this.originDesignerProps);
  //   } else {
  //     Object.assign(finallyDesignerProps, designerProps, this.originDesignerProps);
  //   }
  //   const display = this.props?.style?.display;

  //   if (display) {
  //     finallyDesignerProps.inlineLayout = display === 'inline' || display === 'inline-block';
  //   }
  ...
  get previous() {
    ...
    return this.parent.children[this.index - 1] as any;
  }
  ...
  get childrens(): TreeNode[] {
    return this.children.reduce((buf, node) => {
      return buf.concat(node as any).concat(node.childrens as TreeNode[]);
    }, []);
  }
