...
const useGridLayout = (outerProps: IFormGridProps): ILayout => {
  ...
  const calculateSmartColumns = (target: HTMLElement) => {
    ...
    const takeColwrap = (): boolean => {
      return props.current.colWrap?.[index] || (props.current.colWrap as any) || true;
    };
    ...
    const takeMinWidth = () => {
      ...
      if (isValid(props.current.minWidth)) {
        ...
        if (isValid(rMaxColumns)) {
          return Math.floor(
            (clientWidth - (rMaxColumns - 1) * props.current.columnGap) / rMaxColumns,
          );
        } else {
          return 0;
        }
      ...
      if (isValid(props.current.maxWidth)) {
        ...
        if (isValid(props.current.minColumns?.[index])) {
          const calculated = Math.floor(
            (clientWidth - (props.current.minColumns[index] - 1) * props.current.columnGap) /
...
const getStyle = (props: IStyleProps): IStyle => {
  ...
  const spans = Array.from(ref.current?.childNodes || []).reduce((buf, cur: HTMLElement) => {
    const span = isValid(maxColumns)
      ? Math.min((Number(cur.getAttribute('data-span')) || 1) as number, maxColumns)
...
export const FormGrid: ComposedFormGrid = (props) => {
  ...
  return (
    <FormGridContext.Provider value={{ columnGap: props.columnGap, ...layoutParams }}>
      <div {...dataProps} className={cls(`${formGridPrefixCls}-layout`)} style={{...styles,display:'grid'}} ref={ref}>
        {children}
      </div>
